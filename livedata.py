...
while True:
    for line in lines:
        ...
        #print(responsetimes)
        print("Hostname: ",line)
        print("Maximum ping time is", max(responsetimes))
        print("Minimum ping time is", min(responsetimes))
        ...
        f.write("Minumum Response:  " + min(responsetimes) + "ms" + "\n")
        f.write("Maximum Response:  " + max(responsetimes) + "ms" + "\n")
        f.write("Average Response:  " + average[0] + "ms" + "\n")
        f.write("Packet Loss:  " + loss[0] + " packets" + "\n")
        f.write("\n")
        f.close()
        
        #t1=datetime.now().strftime('%d/%m/%Y %H:%M')
        t1=time.time()
